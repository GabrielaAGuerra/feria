    ...
    vistas: {
        ...
        inscripciones: {
            ...
            render: () => `
                ...
                ${componentes.tabla({ columnas: [{ titulo: 'Emprendedor', campo: 'emprendedor' }, { titulo: 'Feria', campo: 'feria' }, { titulo: 'Fecha', campo: 'fecha' }, { titulo: 'Puesto', campo: 'puesto' }, { titulo: 'Pago', campo: 'estadoPago' }, { titulo: 'Acciones', campo: 'acciones' }], filas: inscripcionesConNombre() })}
            `,
...
function conectarEmprendedores() {
    const enlazar = () => {
        ...
        document.querySelectorAll('[data-eliminar-emprendedor]').forEach((btn) => btn.addEventListener('click', () => { app.eliminarRegistro('emprendedores', btn.dataset.eliminarEmprendedor); router.ir('emprendedores'); }));
    };
...
function conectarInscripciones() { document.getElementById('formInscripcion')?.addEventListener('submit', (evento) => { evento.preventDefault(); const data = Object.fromEntries(new FormData(evento.target)); data.puesto = Number(data.puesto); app.agregarRegistro('inscripciones', data); router.ir('inscripciones'); }); document.querySelectorAll('[data-eliminar-inscripcion]').forEach((btn) => btn.addEventListener('click', () => { app.eliminarRegistro('inscripciones', btn.dataset.eliminarInscripcion); router.ir('inscripciones'); })); document.querySelectorAll('[data-editar-inscripcion]').forEach((btn) => btn.addEventListener('click', () => abrirInscripcion(btn.dataset.editarInscripcion))); }
...
function conectarFerias() { document.getElementById('formFeria')?.addEventListener('submit', (evento) => { evento.preventDefault(); app.agregarRegistro('ferias', Object.fromEntries(new FormData(evento.target))); router.ir('ferias'); }); document.querySelectorAll('[data-duplicar]').forEach((btn) => btn.addEventListener('click', () => { app.duplicarFeria(btn.dataset.duplicar); router.ir('ferias'); })); document.querySelectorAll('[data-eliminar-feria]').forEach((btn) => btn.addEventListener('click', () => { app.eliminarRegistro('ferias', btn.dataset.eliminarFeria); router.ir('ferias'); })); document.querySelectorAll('[data-editar-feria]').forEach((btn) => btn.addEventListener('click', () => abrirFeria(btn.dataset.editarFeria))); }
...
function conectarContabilidad() { document.querySelectorAll('[data-eliminar-movimiento]').forEach((btn) => btn.addEventListener('click', () => { app.eliminarRegistro('movimientos', btn.dataset.eliminarMovimiento); router.ir('contabilidad'); })); document.querySelectorAll('[data-editar-movimiento]').forEach((btn) => btn.addEventListener('click', () => abrirMovimiento(btn.dataset.editarMovimiento))); }
